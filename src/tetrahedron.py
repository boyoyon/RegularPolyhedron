...
WIN_TITLE = "Tetrahedron"  # ウィンドウのタイトル / Window title
...
points = np.empty((4, 3), np.float32)

faces = np.empty((4, 3), np.int32)
...
def createTexture(c, moji):
    ...
    cv2.line(texture, (W // 2, 0), (0, H - 1), c, 3)
    cv2.line(texture, (W // 2, 0), (W - 1, H - 1), c, 3)
    cv2.line(texture, (0, H - 1), (W - 1, H - 1), c, 3)
    ...
    cv2.putText(texture, text = moji, org = (105,180), fontFace = fontFace, fontScale = 2.0, 
            color = c, thickness = 6, lineType = cv2.LINE_4) 
...
def main():
    ...
    print(':')
    print('python %s <texture image_1> <texture image_2> ... <texture image_4>' % argv[0])
